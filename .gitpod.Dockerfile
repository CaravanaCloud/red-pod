...
RUN bash -c "mkdir -p '/tmp/oc' && wget -nv -O '/tmp/oc/oc-mirror.tar.gz' 'https://mirror.openshift.com/pub/openshift-v4/x86_64/clients/ocp/latest/oc-mirror.tar.gz' && tar zxvf '/tmp/oc/oc-mirror.tar.gz' -C '/tmp/oc' && sudo mv '/tmp/oc/oc' '/usr/local/bin/' && sudo mv '/tmp/oc/kubectl' '/usr/local/bin/' && rm '/tmp/oc/oc-mirror.tar.gz'"
